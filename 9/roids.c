...
void ship_draw( const ship_t* ship )
{
    ...
    if( (ship->x + dx < 0 || ship->x + lx < 0 || ship->x + rx < 0)  && (ship->y + dy < 0 || ship->y + ly < 0 || ship->y + ry < 0) ){
    	draw_triangle( ship->x + dx + 1, ship->y + dy + 1,
            ship->x + lx + 1, ship->y + ly + 1,
            ship->x + rx + 1, ship->y + ry + 1,
            shipcolor );
    }
    if( (ship->x + dx > 1 || ship->x + lx > 1 || ship->x + rx > 1) && (ship->y + dy < 0 || ship->y + ly < 0 || ship->y + ry < 0) ){
    	draw_triangle( ship->x + dx - 1, ship->y + dy + 1,
            ship->x + lx - 1, ship->y + ly + 1,
            ship->x + rx - 1, ship->y + ry + 1,
            shipcolor );
    }
    if ( (ship->x + dx < 0 || ship->x + lx < 0 || ship->x + rx < 0) && (ship->y + dy > 1 || ship->y + ly > 1 || ship->y + ry > 1) ){
    	draw_triangle( ship->x + dx + 1, ship->y + dy - 1,
            ship->x + lx + 1, ship->y + ly - 1,
            ship->x + rx + 1, ship->y + ry - 1,
            shipcolor );
    }
    if ( (ship->x + dx > 1 || ship->x + lx > 1 || ship->x + rx > 1) && (ship->y + dy > 1 || ship->y + ly > 1 || ship->y + ry > 1) ){
    	draw_triangle( ship->x + dx - 1, ship->y + dy - 1,
            ship->x + lx - 1, ship->y + ly - 1,
            ship->x + rx - 1, ship->y + ry - 1,
            shipcolor );
    ...
    if ( ship->x + dx < 0 || ship->x + lx < 0 || ship->x + rx < 0 ){
    ...
    if ( ship->y + dy < 0 || ship->y + ly < 0 || ship->y + ry < 0 ){
    	draw_triangle( ship->x + dx, ship->y + dy + 1,
            ship->x + lx, ship->y + ly + 1,
            ship->x + rx, ship->y + ry + 1,
            shipcolor );
    }
    if ( ship->x + dx > 1 || ship->x + lx > 1 || ship->x + rx > 1 ){
    	draw_triangle( ship->x + dx - 1, ship->y + dy,
            ship->x + lx - 1, ship->y + ly,
            ship->x + rx - 1, ship->y + ry,
            shipcolor );
    }
    if ( ship->y + dy > 1 || ship->y + ly > 1 || ship->y + ry > 1 ){
    	draw_triangle( ship->x + dx, ship->y + dy - 1,
            ship->x + lx, ship->y + ly - 1,
            ship->x + rx, ship->y + ry - 1,
            shipcolor );
   ...
   // printf("dx: %f dy: %f lx: %f ly: %f rx: %f ry: %f\n", dx, dy, lx, ly, rx, ry);
    /* TASK 3 */
    /* TODO: 
...
roid_t roid_create( void )
{
    ...
    roid.x = 1;//drand48();
    roid.y = 1;//drand48();
    roid.dx = 0;//(drand48() * 2.0 * ROID_SPEED_MAX) - ROID_SPEED_MAX;
    roid.dy = 0;//(drand48() * 2.0 * ROID_SPEED_MAX) - ROID_SPEED_MAX;
...
void roid_draw( const roid_t* roid )
{
    ...
    if ( (roid->x-roid->width/2.0 < 0 || roid->x+roid->width/2.0 < 0) && (roid->y-roid->height/2.0 < 0 || roid->y+roid->height/2 < 0) ){
    	draw_rectangle( 1 + roid->x-roid->width/2.0, 1 + roid->y-roid->height/2.0, 
            1 + roid->x+roid->width/2.0, 1 + roid->y+roid->height/2,
            roid->color );
    }

    if ( (roid->x-roid->width/2.0 < 0 || roid->x+roid->width/2.0 < 0) && (roid->y-roid->height/2.0 > 1 || roid->y+roid->height/2 > 1) ){
    	draw_rectangle( 1 + roid->x-roid->width/2.0, (roid->y-roid->height/2.0) - 1, 
            1 + roid->x+roid->width/2.0, (roid->y+roid->height/2) - 1,
            roid->color );
    }

    if ( (roid->x-roid->width/2.0 > 1 || roid->x+roid->width/2.0 > 1) && (roid->y-roid->height/2.0 < 0 || roid->y+roid->height/2 < 0) ){
    	draw_rectangle( (roid->x-roid->width/2.0) - 1, 1 + roid->y-roid->height/2.0, 
            (roid->x+roid->width/2.0) - 1, 1 + roid->y+roid->height/2,
            roid->color );
    }

    if ( (roid->x-roid->width/2.0 > 1 || roid->x+roid->width/2.0 > 1) && (roid->y-roid->height/2.0 > 1 || roid->y+roid->height/2 > 1) ){
    	draw_rectangle( (roid->x-roid->width/2.0) - 1, (roid->y-roid->height/2.0) - 1, 
            (roid->x+roid->width/2.0) - 1, (roid->y+roid->height/2) - 1,
            roid->color );
    }

    if ( (roid->x-roid->width/2.0 < 0 || roid->x+roid->width/2.0 < 0) ){
    	draw_rectangle( 1 + roid->x-roid->width/2.0, roid->y-roid->height/2.0, 
            1 + roid->x+roid->width/2.0, roid->y+roid->height/2,
            roid->color );
    }

    if ( (roid->x-roid->width/2.0 > 1 || roid->x+roid->width/2.0 > 1) ){
    	draw_rectangle( (roid->x-roid->width/2.0) - 1, roid->y-roid->height/2.0, 
            (roid->x+roid->width/2.0) - 1, roid->y+roid->height/2,
            roid->color );
    }

    if ( (roid->y-roid->height/2.0 < 0 || roid->y+roid->height/2 < 0) ){
    	draw_rectangle( roid->x-roid->width/2.0, 1 + roid->y-roid->height/2.0, 
            roid->x+roid->width/2.0, 1 + roid->y+roid->height/2,
            roid->color );
    }

    if ( (roid->y-roid->height/2.0 > 1 || roid->y+roid->height/2 > 1) ){
...
if( ( x >= x1 && x <= x2) || (x >= x1 && x2 >= 1 && x <= x2-1) || (x1 <= 0 && x >= 1+x1 && x <= x2)){
	...
	if ( (roid->x - roid->width/2 <= 0 || roid->x + roid->width/2 <= 0) && (roid->y - roid->height/2 <= 0 || roid->y + roid->height/2 <= 0) ){
   		if( (shot->x >= roid->x - roid->width/2 +1 &&
            shot->x <= roid->x + roid->width/2 +1 &&
            shot->y >= roid->y - roid->height/2 +1&&
            shot->y <= roid->y + roid->height/2 +1) || ( shot->x >= roid->x - roid->width/2 &&
	...
	if ( (roid->x - roid->width/2 <= 0 || roid->x + roid->width/2 <= 0) && (roid->y - roid->height/2 >= 1 || roid->y + roid->height/2 >= 1) ){
		if( (shot->x  >= roid->x - roid->width/2 +1 &&
            shot->x  <= roid->x + roid->width/2 +1 &&
            shot->y  >= roid->y - roid->height/2 -1 &&
            shot->y  <= roid->y + roid->height/2 -1 ) || ( shot->x >= roid->x - roid->width/2 &&
	...
	if ( (roid->x - roid->width/2 >= 1 || roid->x + roid->width/2 >= 1) && (roid->y - roid->height/2 <= 0 || roid->y + roid->height/2 <= 0) ){
    	if( (shot->x  >= roid->x - roid->width/2 -1 &&
            shot->x  <= roid->x + roid->width/2 -1 &&
            shot->y  >= roid->y - roid->height/2 +1 &&
            shot->y  <= roid->y + roid->height/2 +1) || ( shot->x >= roid->x - roid->width/2 &&
	...
	if ( (roid->x - roid->width/2 >= 1 || roid->x + roid->width/2 >= 1) && (roid->y - roid->height/2 >= 1 || roid->y + roid->height/2 >= 1) ){
    	if( (shot->x  >= roid->x - roid->width/2 -1 &&
            shot->x  <= roid->x + roid->width/2 -1 &&
            shot->y  >= roid->y - roid->height/2 -1 &&
            shot->y  <= roid->y + roid->height/2 -1) || ( shot->x >= roid->x - roid->width/2 &&
	...
	if ( roid->x - roid->width/2 <= 0 || roid->x + roid->width/2 <= 0){
		if( (shot->x  >= roid->x - roid->width/2 + 1 &&
            shot->x  <= roid->x + roid->width/2 + 1 &&
            shot->y >= roid->y - roid->height/2 &&
            shot->y <= roid->y + roid->height/2) || ( shot->x >= roid->x - roid->width/2 &&
            shot->x <= roid->x + roid->width/2 &&
            shot->y >= roid->y - roid->height/2 &&
            shot->y <= roid->y + roid->height/2 ) ){
			return 1;
	...
	if ( roid->x - roid->width/2 >= 1 || roid->x + roid->width/2 >= 1){
		if( (shot->x  >= roid->x - roid->width/2 - 1 &&
            shot->x  <= roid->x + roid->width/2 - 1 &&
            shot->y >= roid->y - roid->height/2 &&
            shot->y <= roid->y + roid->height/2) || ( shot->x >= roid->x - roid->width/2 &&
	...
	if ( roid->y - roid->height/2 <= 0 || roid->y + roid->height/2 <= 0 ){
	...
	if ( roid->y - roid->height/2 >= 1 || roid->y + roid->height/2 >= 1 ){
		if( (shot->x  >= roid->x - roid->width/2 &&
            shot->x  <= roid->x + roid->width/2 &&
            shot->y >= roid->y - roid->height/2 -1 &&
            shot->y <= roid->y + roid->height/2 -1) || ( shot->x >= roid->x - roid->width/2 &&
...
int main(int argc, char** argv)
{
    ...
    while( 1 )
    {
        ...
        if( numroids == 0 )
        {
            level++;
            printf( "LEVEL %lu\n", level );
            roids_add(1);//level * ROIDS_PER_LEVEL ); // create new roids * EDITED<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
        }
